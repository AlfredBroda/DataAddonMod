...
![DataAddonMerge preview](images/preview.png)
...
```text
DataAddonMod/
|-- DataAddonMerge.csproj       .NET Framework 4.8 BepInEx project
...
These paths are imported by `DataAddonMerge.csproj` and are intentionally kept outside the project file because they are machine-specific.
...
```bash
dotnet msbuild DataAddonMerge.csproj -getProperty:DependsDir -getProperty:BepInExDir
```
...
```bash
dotnet msbuild DataAddonMerge.csproj -preprocess:project.evaluated.xml
```
...
The compiled plugin is written beneath `bin/`. Copy `DataAddonMerge.dll` to the game's BepInEx plugins directory, normally:
...
```text
Ostranauts/Ostranauts_Data/Mods/DataAddon/
|-- mod_info.json
|-- preview.png
`-- plugins/
    `-- DataAddonMerge.dll
...
Use `--dry-run` to inspect the package paths without copying:
